...
tidy.d <- d %>%
  ...
  CashS_NW) %>%
  na.omit(cols=CashS_W, CashS_NW) %>% # eligibility, n=55
...
df.btw <- d %>%
  rename(ID = V1) %>%
  select(ID, S_UBISupport, D_UBISupport)

df.btw$Condition <- NA
df.btw$Condition[!is.na(df.btw$S_UBISupport)] <- "Secure"
df.btw$Condition[!is.na(df.btw$D_UBISupport)] <- "Deserve"


#summary(lm(support~Condition, data=df.btw))
test.support = t.test(df.btw$S_UBISupport, df.btw$D_UBISupport, 
               var.equal = TRUE, 
               paired = FALSE)
test.support
```
...
tidy.deserve <- d %>%
  ...
  na.omit(cols=D.UbiS_W, D.UbiR_W, D.UbiS_NW, D.UbiR_NW) %>%
  select(ID, D.UbiS_W, D.UbiR_W, D.UbiS_NW, D.UbiR_NW) %>%
...
?t.test
df_S.UbiS <- tidy.deserve[tidy.deserve$Question=="S.UbiS",]
