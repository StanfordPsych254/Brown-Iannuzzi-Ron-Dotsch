...
The planned sample size is 58 with a goal of having 55 participants complete the study. There will be no termination rule. The only exclusion rule will be whether a participant fails to commit to carefully reading the content of the experiment, whether they fail the instructional manipulation check, if select the same response option across all items, and if they report being distracted during the study.  
...
####Import data	
```{r loaddf}
d <- read_csv("../data/pilot_data.csv", na = c("", "NA")) 
``` 
